...
import { initIdTokenHandler, loggedInEventName, loggedOutEventName, addCallback as addAuthCallback } from './auth';
...
function _newConnection() {
    socket = io(window.location.protocol + '//' + window.location.host, { secure });
...
async function _createAuthListener(socket) {
    ...
    socket.on('unauthorized', () => {
        ...
        auth.refreshIdTokenAsync().then(([_, idToken]) => {
            console.info('calling authenticate after userTokens.refreshIdTokenAsync in services.socketIO.js', idToken);
            _authenticate(socket, idToken);
        }).catch(() => {
            refreshTimeout = setTimeout((socket) => {
                _authenticate(socket, auth.getToken());
                refreshTimeout = null;
            }, 5000, socket);
        });
